...
4. [Online Double-Spending Mitigation](#4-online-double-spending-mitigation)
5. [Offline Double-Spending Mitigation](#5-offline-double-spending-mitigation)
6. [Threat Vector A — Replay Attacks](#6-threat-vector-a--replay-attacks)
7. [Threat Vector B — Multi-Terminal Offline Over-Spending](#7-threat-vector-b--multi-terminal-offline-over-spending)
8. [Escrow-Bound Offline Authorization](#8-escrow-bound-offline-authorization)
9. [Escrow Token](#9-escrow-token)
10. [Escrow Lifecycle](#10-escrow-lifecycle)
11. [Transaction Identity vs Transport Identity](#11-transaction-identity-vs-transport-identity)
12. [Transaction Lifecycle](#12-transaction-lifecycle)
13. [Proposed OLOS V3 Components](#13-proposed-olos-v3-components)
14. [Component Responsibilities](#14-component-responsibilities)
15. [Event Bus](#15-event-bus)
16. [Fault Codes](#16-fault-codes)
17. [Security Model](#17-security-model)
18. [Double-Spending Model](#18-double-spending-model)
19. [Critical Security Principle](#19-critical-security-principle)
20. [Security Test Suite](#20-security-test-suite)
21. [Example End-to-End Scenario](#21-example-end-to-end-scenario)
22. [Example Replay Scenario](#22-example-replay-scenario)
23. [Example Escrow Exhaustion](#23-example-escrow-exhaustion)
24. [V3 Prototype Limitations](#24-v3-prototype-limitations)
25. [Recommended V3 Research Questions](#25-recommended-v3-research-questions)
26. [V3 Development Roadmap](#26-v3-development-roadmap)
27. [Project Status](#27-project-status)
28. [Core Proposition](#28-core-proposition)
...
**Deterministic compute**
Transaction and rules processing is intended to be deterministic so that identical inputs produce identical state transitions. The production architecture may use isolated execution environments such as MicroVMs for rules execution.

**Settlement Core finality**
The authoritative settlement state tracks transaction or claim state. Example states: `PENDING`, `REDEEMED`, `SETTLED`.
...
`REJECTED` · `REPLAYED` · `TAMPERED` · `REVOKED_DEVICE` · `ESCROW_EXCEEDED` · `ESCROW_EXPIRED` · `ESCROW_CONFLICT` · `COUNTER_ROLLBACK` · `DUPLICATE` · `SETTLEMENT_CONFLICT`
...
5. Terminal verifies: device authorized, escrow valid, £50 ≤ £200.
...
**Secure hardware**
The prototype does not prove that a terminal private key cannot be extracted. A production system may require Secure Elements, HSMs, Trusted Execution Environments, hardware-backed key storage, and device attestation.
